...
export default function CurationSection({ locale }: { locale?: string }) {
  ...
  const categories: CategoryData[] = [
    {
      ...
      itemsKo: ["클렌징", "토너·미스트", "에센스·세럼", "앰플", "모이스처라이저", "선케어", "마스크", "패치", "아이케어"],
      itemsEn: ["Cleansers", "Toner & Mist", "Essence & Serum", "Ampoules", "Moisturizers", "Sun Care", "Masks & Sheets", "Acne Patches", "Eye Care"],
    ...
    },
    {
      ...
      itemsKo: ["샴푸·클렌징", "컨디셔너", "트리트먼트", "스칼프 케어", "스타일링", "헤어 에센스", "헤어 컬러"],
      itemsEn: ["Shampoo & Cleanse", "Conditioners", "Treatments", "Scalp Care", "Styling Products", "Hair Oils & Essence", "Hair Color"],
    ...
    },
    {
      ...
      itemsKo: ["베이스 메이크업", "페이스 컬러", "아이 메이크업", "아이브로우", "립 메이크업"],
      itemsEn: ["Base Makeup", "Face Palettes & Blush", "Eye Makeup", "Eyebrows", "Lip Makeup"],
    ...
    },
    {
      ...
      itemsKo: ["바디 클렌징", "바디 모이스처라이저", "각질 케어", "핸드·풋 케어", "데오도란트"],
      itemsEn: ["Body Wash", "Body Lotion & Creams", "Exfoliators & Scrubs", "Hand & Foot Care", "Deodorants"],
    ...
    },
    {
      ...
      itemsKo: ["오럴 케어", "여성 위생", "남성 그루밍", "쉐이빙", "위생 케어"],
      itemsEn: ["Oral Care", "Feminine Hygiene", "Men's Grooming", "Shaving Essentials", "Sanitary Care"],
      tags: ["Balanced Core", "Essential Mix", "Price Fit"]
    },
    {
      nameKo: "뷰티 툴",
      ...
      itemsKo: ["페이스 툴", "메이크업 툴", "헤어 툴", "바디 툴", "보관·액세서리"],
      itemsEn: ["Face Tools & Rollers", "Makeup Brushes & Sponges", "Hair Brushes & Tools", "Body Brushes", "Storage & Cases"],
  ...
  const standardsEn = [
    ...
    "Store Fit · Demographic Match",
    "Price Fit · Bestselling Tiers",
    "Retail Margin · 45-60% Gross Margin",
    "Store Profile Match · Localized Mix"
  ];

  return (
    <section
      id="products"
      className="w-full bg-[#141414] border-y border-[#2a2a2a] select-none text-left font-sans font-medium"
        ...
        <div className="grid lg:grid-cols-[0.43fr_0.57fr] gap-12 items-stretch">
          ...
          <div className="flex flex-col justify-between gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <h2 className="font-display text-3xl sm:text-[38px] font-bold leading-[1.25] text-white my-2 tracking-tight keep-all">
                {isKo ? (
                  <>모든 제품을 공급하지 않습니다.<br />당신의 매장에 맞는 제품을 큐레이션합니다.</>
                ) : (
                  <>We Don't List Everything.<br />We Select What Belongs in Your Store.</>
                )}
              </h2>
              <p className="text-[13.5px] text-[#9ca3af] leading-relaxed max-w-xl my-2 font-medium">
                {isKo 
                  ...
                  : "We evaluate ingredient safety, gross margin profiles, and local shopper demographics to select only the most profitable and high-converting K-Beauty essentials."
                }
              </p>

              {/* Core Selection Standards Tags */}
              <div className="flex flex-wrap gap-2.5 my-3">
                {(isKo ? standardsKo : standardsEn).map((std) => (
                  <span
                    key={std}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2 text-[11px] text-[#7A7A7A] font-semibold pl-1 mt-1">
                ...
                <span>
                  {isKo 
                    ? "복잡한 상품 선정은 K SELECT HUB가 맡고, 리테일러는 판매와 성장에 집중합니다."
                    : "Let K SELECT HUB handle product curation so you can focus on sales and growth."
                  }
                </span>
              </div>
            </div>
          ...
          <div className="relative rounded-[24px] border border-white/10 bg-[#0c0c0c] shadow-2xl flex flex-col justify-between overflow-hidden p-8 sm:p-10 min-h-[480px]">
            ...
            <div className="absolute inset-0 z-0">
              {categories.map((cat, index) => (
                ...
                >
                  <Image
                    src={cat.image}
                    alt={`${cat.nameEn} Curation Visual`}
                    fill
            ...
            <div className="relative z-20 flex justify-between items-center w-full bg-black/40 backdrop-blur-sm border border-white/5 rounded-[12px] p-4.5">
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[#ff2b75] tracking-widest font-black uppercase font-display">
                  ACTIVE SHOWCASE
                </span>
            ...
            <div className="relative z-20 bg-black/60 backdrop-blur-md border border-white/10 rounded-[18px] p-5 flex flex-col gap-4 text-left">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[9.5px] text-[#ff2b75] font-black uppercase tracking-[0.12em] block font-display">
                  {isKo ? `${categories[activeCategory].nameKo} 핵심 제품군` : `${categories[activeCategory].nameEn} Bestsellers`}
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {(isKo ? categories[activeCategory].itemsKo : categories[activeCategory].itemsEn).map((item) => (
                    <span
                      key={item}
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[9.5px] text-[#FAFAFA]/50 tracking-widest font-black uppercase font-display">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {categories[activeCategory].tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[9.5px] font-bold text-[#ff2b75] bg-[#ff2b75]/8 border border-[#ff2b75]/20 px-2.5 py-0.5 rounded-[4px] tracking-wide font-display"
                    >
                      {tag}
                    </span>
                  ))}
